  ...
  def _execute_transcribe_stage(
    ...
    *,
    ...
    transcribe_decision = next(
      decision for decision in run.stage_decisions if decision.stage == PipelineStage.TRANSCRIBE
    ).decision
    metadata = run.metadata
    ...
    self._metadata_repository.write(metadata)
    fallback_suffix = f" - {result.device_fallback_reason}" if result.device_fallback_reason else ""
    self._artifact_planner.append_log_event(
      run.artifacts.root_dir,
