...
class PromptRunExecutor:
  ...
  def execute(
    ...
    if report_decision is None:
      raise PromptRenderPrerequisiteError("Prompt rendering could not resolve the report stage.")
  ...
  def _render_and_persist_prompt(
    ...
    transcript_text = prepared_run.artifacts.transcript_raw.read_text(encoding="utf-8").strip()
