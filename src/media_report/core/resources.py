...
def _resource_names(package: str, suffix: str) -> list[str]:
  root = files(package)
  return sorted(
    item.name for item in root.iterdir() if item.is_file() and item.name.endswith(suffix)
  )


def list_prompt_templates() -> list[str]:
  return [
    name.removesuffix(".md") for name in _resource_names("media_report.templates.prompts", ".md")
  ]
