...
def process_command(
  ...
  resume: Annotated[
    ...
    typer.Option(
      "--resume",
      help=("Reuse a valid sibling artifact directory and execute only the stages still needed."),
    ),
  ] = False,
  overwrite: Annotated[
    bool,
    typer.Option(
      "--overwrite",
      help=(
        "Deprecated alias for --resume in Sprint 2. A destructive overwrite is not exposed yet."
      ),
    ),
  ] = False,
  provider: Annotated[
    ...
    typer.Option(
      "--provider",
      help=("Override the planned LLM provider recorded in metadata and used for remote warnings."),
    ),
  ] = None,
  model: Annotated[
    str | None,
    typer.Option("--model", help="Override the planned LLM model recorded in pipeline metadata."),
  ] = None,
  language: Annotated[
    str | None,
    typer.Option(
  ...
  template: Annotated[
    str,
    typer.Option("--template", help="Choose the prompt template name stored in pipeline metadata."),
  ] = "generic",
